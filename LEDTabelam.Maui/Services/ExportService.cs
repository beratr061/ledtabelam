...
/// <summary>
/// Dışa aktarma servisi implementasyonu
/// </summary>
public class ExportService : IExportService
{
    ...
    private bool WriteAnimatedGif(Stream stream, IReadOnlyList<SKBitmap> frames, int frameDelayMs)
    {
        ...
        foreach (var frame in frames)
            WriteGifFrame(stream, frame, frameDelayMs / 10, palette);
    ...
    private void WriteGifFrame(Stream stream, SKBitmap bitmap, int delayCentiseconds, SKColor[] palette)
    {
        ...
        WriteUInt16(stream, 0);
        WriteUInt16(stream, (ushort)bitmap.Width);
        WriteUInt16(stream, (ushort)bitmap.Height);
        stream.WriteByte(0x00);
    ...
    private SKColor[] BuildOptimalPalette(IReadOnlyList<SKBitmap> frames)
    {
        ...
        int sampleStep = Math.Max(1, frames.Count / 10);

        for (int f = 0; f < frames.Count; f += sampleStep)
        {
            var frame = frames[f];
            int pixelStep = Math.Max(1, (frame.Width * frame.Height) / 10000);
